...
public class InformationUserDAO {
	...
	public static void addUser(String id, String email, String fName, String lName, String facultyID, String classID,
			String pass) {
		try {
			Connection conn = MyConnection.getConnection();
			...
			pr.setDate(5, null);
			pr.setString(6, facultyID);
			pr.setString(7, classID);
			pr.setString(8, pass);
			pr.setString(9, "Khong co");
			ResultSet rs = pr.executeQuery();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
